/**
 * Gerador de PDF — Relatório de Extrato de Cobranças
...
export interface ExtratoRowPDF {
  cobrancaId: number;
...
export interface RelatorioExtratoPDFInput {
...
export async function gerarRelatorioExtratoPDF(input: RelatorioExtratoPDFInput): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    const drawPageHeader = () => {
      ...
      doc.fontSize(11).font("Helvetica-Bold").fillColor("#ffffff")
        .text("RELATÓRIO DE EXTRATO", MARGIN, 16, { width: CONTENT_W, align: "right" });
    ...
    const cards = [
      { label: "Total Cobrado", value: fmtBRL(input.totais.totalCobrado), color: COLOR_PRIMARY },
      { label: "Total Pago", value: fmtBRL(input.totais.totalPago), color: COLOR_ACCENT },
      { label: "Total Pendente", value: fmtBRL(input.totais.totalPendente), color: "#c0392b" },
    ];
    ...
    cards.forEach((card, i) => {
      ...
      doc.fontSize(13).font("Helvetica-Bold").fillColor(card.color).text(`R$ ${card.value}`, cx + 8, y + 22, { width: cardW - 16 });
    });
    ...
    // ─── Tabela de registros ──────────────────────────────────────────────────
    const COL = {
      devedor: { x: MARGIN, w: 110 },
      unidade: { x: MARGIN + 110, w: 55 },
      condominio: { x: MARGIN + 165, w: 90 },
      descricao: { x: MARGIN + 255, w: 85 },
      vencimento: { x: MARGIN + 340, w: 60 },
      pagamento: { x: MARGIN + 400, w: 60 },
      valor: { x: MARGIN + 460, w: 55 },
    };
    ...
    const drawTableHeader = () => {
      ...
      const headers = [
        ...
        { label: "Pagamento", col: COL.pagamento },
    ...
    input.rows.forEach((row, idx) => {
      ...
      doc.fontSize(7).font("Helvetica").fillColor(COLOR_TEXT)
        ...
        .text(row.descricao ?? statusLabel, COL.descricao.x + 3, y + 4, { width: COL.descricao.w - 6, ellipsis: true })
        .text(fmtDate(row.dataVencimento), COL.vencimento.x + 3, y + 4, { width: COL.vencimento.w - 6 })
        .fillColor(isPago ? COLOR_ACCENT : COLOR_MUTED)
        .text(isPago ? fmtDate(row.dataPagamento) : statusLabel, COL.pagamento.x + 3, y + 4, { width: COL.pagamento.w - 6 })
    ...
    doc.fontSize(8).font("Helvetica-Bold").fillColor(COLOR_PRIMARY)
      .text(`Total de registros: ${input.rows.length}`, MARGIN + 6, y + 7, { width: CONTENT_W * 0.5 })
      .text(`Total Cobrado: R$ ${fmtBRL(input.totais.totalCobrado)}`, MARGIN, y + 7, { width: CONTENT_W - 6, align: "right" });
    y += 22;
