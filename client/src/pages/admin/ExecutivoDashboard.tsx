...
function AlertaCard({ tipo, titulo, descricao, impacto }: {
  ...
  const styles = {
    critico: { border: "border-red-500/30 bg-red-950/20", icon: <AlertTriangle className="w-4 h-4 text-red-400" />, badge: "bg-red-500/20 text-red-400" },
    aviso: { border: "border-amber-500/30 bg-amber-950/20", icon: <Clock className="w-4 h-4 text-amber-400" />, badge: "bg-amber-500/20 text-amber-400" },
    info: { border: "border-blue-500/30 bg-blue-950/20", icon: <Activity className="w-4 h-4 text-blue-400" />, badge: "bg-blue-500/20 text-blue-400" },
  }[tipo];
  ...
  return (
    <div className={`flex items-start gap-3 p-3 rounded-lg border ${styles.border}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-slate-400 mt-0.5">{descricao}</div>
        {impacto && (
      ...
      <div className="border-b border-slate-800 bg-slate-900/50 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Plataforma de Performance de Recuperação de Crédito</p>
          </div>
          <div className="flex items-center gap-3">
            <Tabs value={periodo} onValueChange={(v) => setPeriodo(v as Periodo)}>
              <TabsList className="bg-slate-800 border border-slate-700">
                <TabsTrigger value="hoje" className="text-xs data-[state=active]:bg-indigo-600">Hoje</TabsTrigger>
                <TabsTrigger value="semana" className="text-xs data-[state=active]:bg-indigo-600">Semana</TabsTrigger>
                <TabsTrigger value="mes" className="text-xs data-[state=active]:bg-indigo-600">Mês</TabsTrigger>
                <TabsTrigger value="trimestre" className="text-xs data-[state=active]:bg-indigo-600">Trimestre</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            {loadingKpis && <RefreshCw className="w-4 h-4 text-slate-500 animate-spin" />}
          </div>
          {loadingKpis ? (
                  ...
                  <div className="space-y-2">
                    {funilData.map((etapa, i) => (
                      <div key={i} className="flex items-center gap-3">
                        <div className="w-36 text-xs text-slate-400 text-right shrink-0">{etapa.name}</div>
                        ...
                        <div className="w-12 text-xs text-slate-400 text-right shrink-0">{etapa.conv}%</div>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <Card className="bg-slate-900 border-slate-800 h-full">
              ...
              <CardContent className="space-y-2">
                {loadingPerdas || loadingEquipe || loadingKpis ? (
                  Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-16 bg-slate-800" />)
                ) : alertas.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    <CheckCircle className="w-8 h-8 text-emerald-400 mb-2" />
                    <div className="text-sm text-emerald-400 font-semibold">Operação saudável</div>
                    <div className="text-xs text-slate-500 mt-1">Nenhum alerta crítico no momento</div>
                  </div>
                ) : (
                  alertas.map((a, i) => (
              ...
              {loadingEquipe ? (
                ...
                <div className="space-y-3">
                  {equipe.slice(0, 5).map((op, i) => (
                    <div key={op.id} className="flex items-center gap-3 p-3 rounded-lg bg-slate-800/50 hover:bg-slate-800 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-semibold text-white truncate">{op.nome}</div>
                        <div className="text-xs text-slate-400">{op.badge}</div>
                      </div>
                      ...
                      <div className="text-right shrink-0 hidden sm:block">
                        <div className="text-sm font-semibold text-white">{op.qtdAcordos}</div>
                        <div className="text-xs text-slate-500">acordos</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {loadingCarteiras ? (
                ...
                <div className="space-y-3">
                  {carteiras.slice(0, 5).map((c, i) => (
                    <div key={c.id} className="p-3 rounded-lg bg-slate-800/50 hover:bg-slate-800 transition-colors">
                      <div className="flex items-center justify-between mb-2">
                        <div className="text-sm font-semibold text-white truncate flex-1 mr-2">{c.nome}</div>
            ...
            {loadingPerdas ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="p-4 rounded-lg bg-amber-950/20 border border-amber-500/20">
                  ...
                  <div className="text-xs text-slate-400 mt-1">Parcelas Atrasadas</div>
                  <div className="text-xs text-amber-400 font-semibold mt-1">{fmt(perdas?.parcelasAtrasadas.valor ?? 0)}</div>
                </div>
                <div className="p-4 rounded-lg bg-slate-800/50 border border-slate-700">
                  <div className="text-2xl font-bold text-slate-300 font-mono">{perdas?.devedoresSemContato ?? 0}</div>
                  <div className="text-xs text-slate-400 mt-1">Sem Contato (30d)</div>
                  <div className="text-xs text-slate-500 mt-1">Devedores ignorados</div>
                </div>
                <div className="p-4 rounded-lg bg-orange-950/20 border border-orange-500/20">
                  ...
                  <div className="text-xs text-slate-400 mt-1">Valor Total em Risco</div>
        ...
        {/* Insights de IA */}
        <Card className="bg-gradient-to-r from-indigo-950/50 to-violet-950/50 border-indigo-800/50">
          <CardHeader className="pb-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {kpis && kpis.taxaRecuperacao > 0 && (
                <div className="p-3 rounded-lg bg-indigo-900/30 border border-indigo-700/30">
                  <div className="text-xs text-indigo-300 font-semibold mb-1">📈 Tendência de Recuperação</div>
              ...
              {funil && funil.length > 2 && funil[1].qtd > 0 && (
                <div className="p-3 rounded-lg bg-violet-900/30 border border-violet-700/30">
                  <div className="text-xs text-violet-300 font-semibold mb-1">🔍 Gargalo do Funil</div>
                  <div className="text-sm text-white">
                    {(() => {
              ...
              {equipe && equipe.length > 0 && (
                <div className="p-3 rounded-lg bg-blue-900/30 border border-blue-700/30">
                  <div className="text-xs text-blue-300 font-semibold mb-1">👥 Performance da Equipe</div>
                  <div className="text-sm text-white">
                    {equipe[0].score >= 70
              ...
              {kpis && (kpis.previsaoReceita ?? 0) > 0 && (
                <div className="p-3 rounded-lg bg-emerald-900/30 border border-emerald-700/30">
                  <div className="text-xs text-emerald-300 font-semibold mb-1">💰 Previsão de Caixa</div>
                  <div className="text-sm text-white">
              ...
              {perdas && perdas.acordosQuebrados.qtd > 0 && (
                <div className="p-3 rounded-lg bg-red-900/30 border border-red-700/30">
                  <div className="text-xs text-red-300 font-semibold mb-1">⚠️ Risco de Perda</div>
                  <div className="text-sm text-white">
              ...
              {kpis && kpis.totalInadimplentes > 0 && (
                <div className="p-3 rounded-lg bg-cyan-900/30 border border-cyan-700/30">
                  <div className="text-xs text-cyan-300 font-semibold mb-1">🎯 Oportunidade de Escala</div>
                  <div className="text-sm text-white">
